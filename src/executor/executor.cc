...
const Executor::
    ExecutorMap
        Executor::execMap_{{OpType::ADD,
                            [](const Instruction &inst, State &state) {
                              executeRegisterRegisterOp(inst, state,
                                                        std::plus<RegVal>());
                            }},
                           {OpType::SUB,
                            [](const Instruction &inst, State &state) {
                           ...
                           {OpType::MUL,
                            [](const Instruction &inst, State &state) {
                              executeRegisterRegisterOp(
                                  inst, state, std::multiplies<RegVal>());
                            }},
                           {OpType::DIV,
                            [](const Instruction &inst, State &state) {
                              auto rs2 = state.regs.get(inst.rs2);
                           ...
                           {OpType::JALR,
                            [](const Instruction &inst, State &state) {
                              ...
                              state.regs.set(inst.rd, state.pc + kXLENInBytes);
                              auto rs1 = state.regs.get(inst.rs1);
                              state.npc = setBit<0, 0>(
                                  rs1 +
                                  inst.imm); // setting the least-significant
                           ...
                           {OpType::ADDI,
                            [](const Instruction &inst, State &state) {
                              executeRegisterImmidiateOp(inst, state,
                                                         std::plus<RegVal>());
                            }},
                           {OpType::ANDI,
                            [](const Instruction &inst, State &state) {
                              executeRegisterImmidiateOp(
                                  inst, state, std::bit_and<RegVal>());
                            }},

                           {OpType::XORI,
                            [](const Instruction &inst, State &state) {
                              executeRegisterImmidiateOp(
                                  inst, state, std::bit_xor<RegVal>());
                            }},
                           ...
                           {OpType::SLTIU,
                            [](const Instruction &inst, State &state) {
                              auto rs1 = state.regs.get(inst.rs1);
                              state.regs.set(inst.rd,
                                             executeSLT(rs1, inst.imm));
                            }},
                           ...
                           {OpType::SRAI,
                            [](const Instruction &inst, State &state) {
                              ...
                              auto res = unsignedCast(
                                  executeSRLT(signedRs1, inst.imm));
                              state.regs.set(inst.rd, res);
                            }},
                           {OpType::SLL,
                            [](const Instruction &inst, State &state) {
                           ...
                           {OpType::BEQ,
                            [](const Instruction &inst, State &state) {
                              executeCondBranch(inst, state,
                                                std::equal_to<RegVal>());
                            }},
                           {OpType::BNE,
                            [](const Instruction &inst, State &state) {
                              executeCondBranch(inst, state,
                                                std::not_equal_to<RegVal>());
                            }},
                           {OpType::BLT,
                            [](const Instruction &inst, State &state) {
                              executeCondBranch(inst, state,
